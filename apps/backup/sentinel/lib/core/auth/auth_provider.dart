...
class SentinelAuthState {
  const SentinelAuthState({
  ...
  bool operator ==(Object other) {
    ...
    return other is SentinelAuthState &&
        other.status == status &&
        other.user == user &&
        other.error == error &&
  ...
  @override
  String toString() {
    return 'SentinelAuthState(status: $status, user: $user, tenantId: $tenantId)';
  }
}

/// Authentication provider using Riverpod
/// Manages guard authentication state and sessions
class AuthNotifier extends StateNotifier<SentinelAuthState> {
  AuthNotifier(this._supabaseClient) : super(const SentinelAuthState());
  ...
  Future<void> initialize() async {
    try {
      state = const SentinelAuthState(status: AuthStatus.loading);

      // Listen to auth state changes
      _supabaseClient.authStateChanges.listen((supabase.AuthState data) {
      ...
      if (currentUser != null && currentSession != null) {
        ...
        state = const SentinelAuthState(status: AuthStatus.unauthenticated);
  ...
  void _handleAuthStateChange(supabase.AuthState authData) {
  ...
  /// Sign in guard with email and password (alias for signInGuard)
  Future<void> signIn({
    required String email,
    required String password,
  }) async {
    await signInGuard(email: email, password: password);
  }
  ...
  Future<void> resetPassword(String email) async {
    try {
      // await _supabaseClient.resetPassword(email); // TODO: Implement
  ...
  Future<void> refreshSession() async {
    try {
      // await _supabaseClient.refreshSession(); // TODO: Implement
...
final authProvider = StateNotifierProvider<AuthNotifier, SentinelAuthState>((ref) {
