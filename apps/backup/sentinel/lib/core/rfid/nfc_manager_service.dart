...
class NfcManagerService {
  ...
  Future<bool> _requestNfcPermission() async {
    try {
      // For NFC permissions on Android, we need to handle them differently
      // NFC permissions are typically declared in the manifest and granted at install time
      // We'll check if we can start a session to verify availability instead
      return true;
    } catch (e) {
    ...
    try {
      ...
      if (ndef != null) {
        ...
        if (cachedMessage != null) {
          ndefData = _processNdefMessage(cachedMessage);
        }
      }
      ...
      final tagData = <String, dynamic>{
        'ndef': ndefData,
        'data': await _extractTagData(tag),
      };
  ...
  Future<Map<String, dynamic>> _extractTagData(NfcTag tag) async {
    ...
    try {
      ...
      if (ndef != null) {
        ...
        // Note: Some Ndef methods may not be available in all versions
        // We'll extract basic information safely
        try {
          final cachedMessage = await ndef.read();
          data['ndefHasMessage'] = cachedMessage.records.isNotEmpty;
        } catch (e) {
          data['ndefHasMessage'] = false;
        }
      } else {
        data['ndefAvailable'] = false;
      }

      // Get available tag data from the tag
      // Extract basic tag information safely
      try {
        final tagData = tag.data;
        data['hasData'] = tagData.isNotEmpty;
      } catch (e) {
        data['hasData'] = false;
      }
  ...
  String? _extractStickerCode(Map<String, dynamic> tagData) {
    try {
      ...
      // Method 2: Try to extract from tag data identifier if available
      final dataSection = tagData['data'] as Map<String, dynamic>? ?? {};
      if (dataSection.containsKey('identifier')) {
        final identifier = dataSection['identifier'] as List<int>?;
        if (identifier != null) {
          final tagIdHex = identifier
